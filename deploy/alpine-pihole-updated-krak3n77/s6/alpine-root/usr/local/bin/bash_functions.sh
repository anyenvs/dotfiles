...
fix_capabilities() {
    ...
    /usr/sbin/capsh --has-p=cap_chown 2>/dev/null && CAP_STR+=',CAP_CHOWN'
    /usr/sbin/capsh --has-p=cap_net_bind_service 2>/dev/null && CAP_STR+=',CAP_NET_BIND_SERVICE'
    /usr/sbin/capsh --has-p=cap_net_raw 2>/dev/null && CAP_STR+=',CAP_NET_RAW'
    /usr/sbin/capsh --has-p=cap_net_admin 2>/dev/null && CAP_STR+=',CAP_NET_ADMIN' || DHCP_READY='false'
    /usr/sbin/capsh --has-p=cap_sys_nice 2>/dev/null && CAP_STR+=',CAP_SYS_NICE'
...
validate_env() {
    ...
    if [[ "$(nc -w1 -z "$FTLCONF_LOCAL_IPV4" 53 2>&1)" != "" ]] && ! ip route get "$FTLCONF_LOCAL_IPV4" > /dev/null ; then
    ...
    if [[ -n "$FTLCONF_LOCAL_IPV6" ]] ; then
        ...
        if [[ "$(nc -w1 -z "$FTLCONF_LOCAL_IPV6" 53 2>&1)" != "" ]] && ! ip route get "$FTLCONF_LOCAL_IPV6" > /dev/null ; then
...
setup_web_php_env() {
    ...
    for config_var in "VIRTUAL_HOST" "CORS_HOSTS" "PHP_ERROR_LOG" "PIHOLE_DOCKER_TAG" "TZ"; do
      ...
      if grep -qE "$beginning_of_line" "$PHP_ENV_CONFIG" ; then
...
setup_web_password() {
    if [ -z "${WEBPASSWORD+x}" ] ; then
        ...
        echo "  [i] Assigning random password: $WEBPASSWORD" | tee /tmp/pihole_password
    else
