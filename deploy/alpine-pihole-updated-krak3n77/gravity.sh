...
[ -n "${DEBUG}" ] && { export PS4='+ [${FUNCNAME##*/}:${LINENO}] '; set -x; };
find / -name gravity.sh -exec sed -i 's/sqlite3 -ni/sqlite3/' {} \;
...
generate_gravity_database() {
  set -x;
  echo "sqlite3 ${gravityDBfile} < ${gravityDBschema}"
