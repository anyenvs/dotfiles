...
test -n "${S6_OVERLAY_VERSION}" && curl -L -s "https://github.com/just-containers/s6-overlay/releases/download/${S6_OVERLAY_VERSION}/s6-overlay-noarch.tar.xz" | tar Jxpf - -C /
test -n "${S6_OVERLAY_VERSION}" && curl -L -s "https://github.com/just-containers/s6-overlay/releases/download/${S6_OVERLAY_VERSION}/s6-overlay-${S6_ARCH}.tar.xz" | tar Jxpf - -C /

ls /init
test -n "${S6_OVERLAY_VERSION}" && mv /init /s6-init
...
#export S6_OVERLAY_VERSION=${S6_OVERLAY_VERSION-v3.1.5.0}
export S6_OVERLAY_VERSION=${S6_OVERLAY_VERSION-v3.1.1.2}
